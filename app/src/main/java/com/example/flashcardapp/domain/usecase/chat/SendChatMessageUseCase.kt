...
class SendChatMessageUseCase(private val repository: ChatRepository) {
    suspend operator fun invoke(userMessage: String, history: List<ChatMessage>, contextMessage: String? = null) =
        repository.sendMessage(userMessage, history, contextMessage)
}
